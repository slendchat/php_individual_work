...
4. **Как работает поиск и фильтрация?**
 ...
 - Передача результата в представление
   ...
   отдаёт в шаблон массив тикетов и текущие значения фильтров (чтобы префиллить форму), а также флаг `isAdmin` для отображения дополнительных действий.
